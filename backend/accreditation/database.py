...
def upload_data(program: str,
    ...
    achievement_level = get_achievement_level(gai_score, question_max)

    DataProcessDAO.insert(term=term, program=program, course=course, gai=gai)
    FacultyCIDAO.insert(course=course, term=term, instr_first_name=instr_first_name,
                        instr_last_name=instr_last_name, assess_title=assess_title, gai_score=gai_score,
                        total_score=total_score, cohort=cohort)
    ProgramCIDAO.insert(gai_score=gai_score, total_score=total_score, achievement_level=achievement_level,
                        cohort=cohort)
    AssessValidityDAO.insert(gai=gai, ga=ga, course=course, question_max=question_max, alignment=alignment,
                             gai_score=gai_score, total_score=total_score, assess_max=assess_max, assess_weight=assess_weight,
                             assess_descript=assess_descript, clos=clos)
    AccredReportDAO.insert(program=program, term=term, ga=ga, gai=gai, assess_type=assess_type, quest_text=quest_text,
                           alignment=alignment, instr_level=instr_level, achievement_level=achievement_level, student_id=student_id)
    
    AnnualReportDAO.insert(program=program, term=term, course=course, ga=ga, gai=gai, student_id=student_id,
                           achievement_level=achievement_level, assess_type=assess_type, instr_comments=instr_comments)
...
def get_flattened_data():
    ...
    for id in ids:
        try:
            ...
            # We just skip exceptions
            continue 

    return results

def get_flattened_data_paginated(page=1, per_page=50):
    data = get_flattened_data()
